...
public class Shoe
{
    ...
    public Shoe(int numberOfDecks)  //Shoe constructor that takes number of deck(s)
    {
        ...
        {
            logger.info("For loop which builds shoe to number of decks entered");
            logger.info("Total decks " + totalDecks + ". Number of decks " + numberOfDecks);
    ...
    public Card pick()
    {
        ...
        if (shoe.size() < 1)
        {
            logger.info("Exception thrown to shoe size going below 1");
        ...
        logger.info("Random deck selected, randomDeck variable is " + randomDeck);
        //Card generated by using pick method from Deck class.
        Card pickCard = (shoe.get(randomDeck)).pick();
        logger.info("Card chosen is " + pickCard.toString());
